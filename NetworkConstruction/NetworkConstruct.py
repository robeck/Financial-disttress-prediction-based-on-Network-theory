...
from NetworkConstruction.RelationConstruct import layer_relation
from NetworkConstruction import Network
...
输出：
    1）自撰写的Network实例
'''
def layer_construct_(data,stocklist,date):
    # 配置logger
    if os.path.exists(rf'/home/haozhic2/ListedCompany_risk/Data/Networkdata/log_{date}.txt'):
        os.remove(rf'/home/haozhic2/ListedCompany_risk/Data/Networkdata/log_{date}.txt')
    else:
        pass
    "logger配置"
    logger = logging.getLogger(__name__)
    logger.setLevel(level=logging.INFO)
    handler = logging.FileHandler(f'/home/haozhic2/ListedCompany_risk/Data/Networkdata/log_{date}.txt')
    formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
    handler.setFormatter(formatter)
    logger.addHandler(handler)

    logger.info(f"We are working on the data in {date}")  # logger 记录

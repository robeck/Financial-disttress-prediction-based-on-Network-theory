...
import time
import logging
from DCC_GARCH.dcc_garch import dcc_relation  # 计算dcc的模型
...
def relation_extract(data,stocki,stockj,logger):
    ...
    data_i = data_i[['Trdwnt','log_return']].rename(columns={'log_return':'returni'}).set_index('Trdwnt')
    data_j = data_j[['Trdwnt','log_return']].rename(columns={'log_return':'returnj'}).set_index('Trdwnt')
    "1. merge一下，将时间匹配"
    df_merge = pd.merge(data_i,data_j,left_on='Trdwnt',right_on='Trdwnt',how='inner')

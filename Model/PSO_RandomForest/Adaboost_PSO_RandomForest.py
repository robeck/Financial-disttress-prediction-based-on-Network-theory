...
from sklearn.feature_selection import RFE # RFE进行特征选择
...
def PSO_RF_Model(data):
    ...
    particle_position_vector = np.array([np.array([random.randint(1, 200),
                                                   random.randint(1, 30),
                                                   random.randint(1, 20)]) for _ in
                                         range(args.n_particles)])  # 初始化每一个粒子的位置
    # 参数2：粒子自身历史最优位置
...
def Model_prediction(params, date, traindata, finaldata, preddata,finposition,features,files,threshold):
    ...
    with open(files, 'a+') as f:
        f.write(f'最佳参数n_estimators,max_depth,min_sample_leaf分别为：{params[0]}，{params[1]}，{params[2]} \n')
    "数据初始化"
    ...
    rfc = RandomForestClassifier(n_estimators=params[0],
                                 ...
                                 n_jobs=-1)
    # Adaboost 随机森林模型
    clf = AdaBoostClassifier(base_estimator=rfc,n_estimators=50,random_state=0)
    ##############################################################################
    "特征选择器进行特征选择"
    # RFE选择器
    ...
    precision = precision_score(test_y,prediction_test,average='binary')
    recall = recall_score(test_y,prediction_test,average='binary')
    ...
    print(f'Adaboost-PSO-RandomForest模型在 样本外测试数据集 的分数是：{scores}')
    print(f'Adaboost-PSO-RandomForest模型在 样本外测试数据集 f1 scores得分为：{f1_scores}')
    print(f'Adaboost-PSO-RandomForest模型在 样本外测试数据集 AUC得分为：{aucscore}')
    print(f'Adaboost-PSO-RandomForest模型在 样本外测试数据集 precision得分为：{precision}')
    print(f'Adaboost-PSO-RandomForest模型在 样本外测试数据集 recall得分为：{recall}')
    print(f'Adaboost-PSO-RandomForest模型在 样本外测试数据集 tpr 得分为：{tpr}')
    print(f'Adaboost-PSO-RandomForest模型在 样本外测试数据集 fpr 得分为：{fpr}')
    print('###########################################################')
    with open(files, 'a+') as f:
        f.write(f'Adaboost-PSO-RandomForest模型筛选后的金融指标为：{Finindicators} \n')
        f.write(f'Adaboost-PSO-RandomForest模型在 样本外测试数据集 的分数是：{scores} \n')
        f.write(f'Adaboost-PSO-RandomForest模型在 样本外测试数据集 f1 scores得分为：{f1_scores} \n')
        f.write(f'Adaboost-PSO-RandomForest模型在 样本外测试数据集 AUC得分为：{aucscore} \n')
        f.write(f'Adaboost-PSO-RandomForest模型在 样本外测试数据集 precision得分为：{precision} \n')
        f.write(f'Adaboost-PSO-RandomForest模型在 样本外测试数据集 recall得分为：{recall} \n')
        f.write(f'Adaboost-PSO-RandomForest模型在 样本外测试数据集 tpr 得分为：{tpr} \n')
        f.write(f'Adaboost-PSO-RandomForest模型在 样本外测试数据集 fpr 得分为：{fpr} \n')
        f.write('########################################################### \n')

    "2.2 相关测试得分输出dict"
    reslist = [scores,f1_scores,aucscore,precision,recall] # 各个结果的list输出，便于统计
    ...
    # plt.savefig(f'/home/haozhic2/ListedCompany_risk/Results_output/Figure_file/Adaboost_PSO_Randomforest_roc_{date}', dpi=300)
    # plt.show()

    "3.2. precison-recall"
    "绘制方案"
    # display = PrecisionRecallDisplay.from_estimator(
    #     clf, pred_X, pred_y, name="Adaboost-PSO-RandomForest"
    # )
    # _ = display.ax_.set_title(f"2-class Precision-Recall curve for PSO-RandomForest on {date}")
    # plt.savefig(f'/home/haozhic2/ListedCompany_risk/Results_output/Figure_file/Adaboost_PSO_Randomforest_precision_recall_{date}', dpi=300)
    # plt.show()

    "3.3 KS绘制"
    # plts = plt_ks(pred_y, pred_prob_pred, 'Adaboost-PSO-RandomForest', date)
    # plts.savefig(f'/home/haozhic2/ListedCompany_risk/Results_output/Figure_file/Adaboost_PSO_Randomforest_KSplot_{date}', dpi=300)
    # plts.show()


    "3.4 机器学习可解释性分析"
    "shap_value: 创建这个解释器"
    # explainer = shap.KernelExplainer(clf.predict_proba,pred_X,link='logit')
    ...
    # shap.summary_plot(shape_value[1], pred_X, title=f'SHAP value of Adaboost-PSO-RandomForest on date {date}')
    # plt.savefig(f'/home/haozhic2/ListedCompany_risk/Results_output/Figure_file/Adaboost_PSO_Randomforest_shap_summary_{date}', dpi=300)
    # plt.show()

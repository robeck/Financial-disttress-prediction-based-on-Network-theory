...
def RF_model(data,finaldata,predicted_data,finposition,features,files,threshold):
    ...
    ######################################################################################################
    "4.最优模型实例化，形成Adaboost RandomForest模型！"
    ...
    #                                          random_state=90)
    #     final_model = AdaBoostClassifier(base_estimator=clf,n_estimators=50,random_state=90) #加入Adaboost
    #     final_model.fit(Train_X, Train_y)
    # else:
    #     final_model = AdaBoostClassifier(base_estimator=Output_model, n_estimators=50, random_state=90)
    #     final_model.fit(Train_X,Train_y)

    clf = RandomForestClassifier(n_estimators=best_parameters.get('n_estimators'),
                                 n_jobs=-1,
                                 max_depth=best_parameters.get('max_depth'),
                                 min_samples_leaf=best_parameters.get('min_samples_leaf'),
                                 criterion=best_parameters.get('criterion'),
    ...
    # def Model_prediction(model,date,predicted_data,files):
    ...
    print(f'Adaboost_Randomforest模型在 样本外预测数据集 的得分为：{scores}')
    print(f'Adaboost_Randomforest模型在 样本外预测数据集 f1 scores得分为：{f1_scores}')
    print(f'Adaboost_Randomforest模型在 样本外预测数据集 的AUC得分为：{auc_scores}')
    print(f'Adaboost_Randomforest模型在 样本外测试数据集 precision得分为：{precision}')
    print(f'Adaboost_Randomforest模型在 样本外测试数据集 recall得分为：{recall}')
    print(f'Adaboost_Randomforest模型在 样本外测试数据集 tpr 得分为：{tpr}')
    print(f'Adaboost_Randomforest模型在 样本外测试数据集 fpr 得分为：{fpr}')
    print('###########################################################')
    with open(files, 'a+') as f:
        f.write(f'Adaboost_Randomforest模型筛选后的金融指标为：{Finindicators} \n')
        f.write(f'Adaboost_Randomforest模型在 样本外预测数据集 的得分为：{scores} \n')
        f.write(f'Adaboost_Randomforest模型在 样本外预测数据集 f1 scores得分为：{f1_scores} \n')
        f.write(f'Adaboost_Randomforest模型在 样本外预测数据集 的AUC得分为：{auc_scores} \n')
        f.write(f'Adaboost_Randomforest模型在 样本外测试数据集 precision得分为：{precision} \n')
        f.write(f'Adaboost_Randomforest模型在 样本外测试数据集 recall得分为：{recall} \n')
        f.write(f'Adaboost_Randomforest模型在 样本外测试数据集 tpr 得分为：{tpr} \n')
        f.write(f'Adaboost_Randomforest模型在 样本外测试数据集 fpr 得分为：{fpr} \n')
        f.write('########################################################### \n')
    ...
    "2.1 sklearn的其他方法"
    # ax = plt.gca()
    # ax.set_title(f'ROC for Adaboost-RandomForest on {date}')
    # rfc_disp = RocCurveDisplay.from_estimator(model, pred_X, pred_y, ax=ax,alpha=0.8)
    # plt.savefig(f'/home/haozhic2/ListedCompany_risk/Results_output/Figure_file/Adaboost_Randomforest_roc_{date}', dpi=300)
    # plt.show()

    "2.2 precison-recall"
    "绘制方案"
    # display = PrecisionRecallDisplay.from_estimator(
    #     model, pred_X, pred_y, name="Adaboost-RandomForest"
    # )
    # _ = display.ax_.set_title(f"2-class Precision-Recall curve for Adaboost-RandomForest on {date}")
    # plt.savefig(
    #     f'/home/haozhic2/ListedCompany_risk/Results_output/Figure_file/Adaboost_Randomforest_precision_recall_{date}',
    #     dpi=300)
    # plt.show()

    "2.3 KS绘制"
    # plts = plt_ks(pred_y, prediction_prob,'Adaboost-RandomForest',date)  # 绘制KS 测试一下！
    # plts.savefig(f'/home/haozhic2/ListedCompany_risk/Results_output/Figure_file/Adaboost_Randomforest_KSplot_{date}',dpi=300)


    "2.4 机器学习可解释性分析"
    "shap_value: 创建这个解释器"
    # explainer = shap.KernelExplainer(model.predict_proba,pred_X,link='logit')
    ...
    # shap.summary_plot(shape_value[1],pred_X,title=f'SHAP value of Adaboost-RandomForest on {date}')
    # plt.savefig(f'/home/haozhic2/ListedCompany_risk/Results_output/Figure_file/Adaboost_Randomforest_shap_summary_{date}',dpi=300)
    # plt.show()

...
"模型评估和数据绘制"
from sklearn.metrics import confusion_matrix,RocCurveDisplay,f1_score,PrecisionRecallDisplay,roc_auc_score # ROC-AUC曲线
...
def best_model(params,date, traindata, preddata,finposition,features):
    ...
    "训练集训练"
    classifer.fit(TrainX, Trainy) #一体化的训练！
    ...
    print(f'PSO-SVM模型在测试数据中的分数：{scores_test_pred}')
    print(f'PSO-SVM模型在测试数据集中的f1 score得分为：{f1_scores}')
    print(f'PSO-SVM模型在测试数据集中的AUC得分为：{auc_scores}')
    with open('/home/haozhic2/ListedCompany_risk/Results_output/Source_file/PSO_SVM_res.txt','a+') as f:
        f.write(f'PSO-SVM模型在测试数据中的分数：{scores_test_pred} \n')
        f.write(f'PSO-SVM模型在测试数据集中的f1 score得分为：{f1_scores} \n')
        f.write(f'PSO-SVM模型在测试数据集中的AUC得分为：{auc_scores} \n')
    ...
    # plt.savefig(f'/home/haozhic2/ListedCompany_risk/Results_output/Figure_file/PSO_SVM_roc_{date}', dpi=300)
    # plt.show()
    ...
    # plt.savefig(
    #     f'/home/haozhic2/ListedCompany_risk/Results_output/Figure_file/PSO_SVM_precision_recall_{date}',
    #     dpi=300)
    # plt.show()

    "KS绘制"
    # plts = plt_ks(predy,y_pred_prob,'PSO-SVM',date) #绘制KS 测试一下！
    # plts.savefig(f'/home/haozhic2/ListedCompany_risk/Results_output/Figure_file/PSO_SVM_KSplot_{date}', dpi=300)
    ...
    # plt.savefig(f'/home/haozhic2/ListedCompany_risk/Results_output/Figure_file/PSO_SVM_shap_summary_{date}',
    #             dpi=300)
    # plt.show()

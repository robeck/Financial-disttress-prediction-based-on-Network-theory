...
from sklearn.ensemble import RandomForestClassifier,AdaBoostClassifier,BaggingClassifier # adaboost算法
...
def best_model(params, date, traindata, preddata,finposition,features):
    ...
    # bagging 学习器
    bagging = BaggingClassifier(base_estimator=clf,n_estimators=20,random_state=66)

    "K折交叉验证-输出结果"
    # scores = cross_val_score(bagging,TrainX,Trainy,cv=5)
    ...
    "2. 样本内测试数据的检验"
    y_test_pred = bagging.predict(testX)
    y_test_prob = bagging.predict_proba(testX)[:, 1]
    "样本内测试数据得分情况"
    scores_test_pred = bagging.score(testX, testy)
    f1_scores = f1_score(testy, y_test_pred, average='binary')
    auc_scores = roc_auc_score(testy, y_test_prob)
    print(f'bagging-PSO-SVM模型在测试数据中的分数：{scores_test_pred}')
    print(f'bagging-PSO-SVM模型在测试数据集中的f1 score得分为：{f1_scores}')
    print(f'bagging-PSO-SVM模型在测试数据集中的AUC得分为：{auc_scores}')
    with open('/home/haozhic2/ListedCompany_risk/Results_output/Source_file/bagging_PSO_SVM_res.txt', 'a+') as f:
        f.write(f'bagging-PSO-SVM模型在测试数据中的分数：{scores_test_pred} \n')
        f.write(f'bagging-PSO-SVM模型在测试数据集中的f1 score得分为：{f1_scores} \n')
        f.write(f'bagging-PSO-SVM模型在测试数据集中的AUC得分为：{auc_scores} \n')

    "3. 样本外预测集的检验"
    "进行预测"
    y_pred_pred = bagging.predict(predX)
    y_pred_prob = bagging.predict_proba(predX)[:, 1]  # 计算X中样本的可能结果的概率
    # print(f'预测的结果为：{y_pred_pred}')
    "各项预测分数"
    score_pred_pred = bagging.score(predX, predy)
    f1_scores = f1_score(predy, y_pred_pred, average='binary')
    auc_scores = roc_auc_score(predy, y_pred_prob)
    print(f'bagging-PSO-SVM模型在预测数据集中的分数为：{score_pred_pred}')
    print(f'bagging-PSO-SVM模型在预测数据集中的f1 scores得分为：{f1_scores}')
    print(f'bagging-PSO-SVM模型在预测数据集中的AUC得分为：{auc_scores}')
    with open('/home/haozhic2/ListedCompany_risk/Results_output/Source_file/bagging_PSO_SVM_res.txt', 'a+') as f:
        f.write(f'bagging-PSO-SVM模型在预测数据集中的分数为：{score_pred_pred} \n')
        f.write(f'bagging-PSO-SVM模型在预测数据集中的f1 scores得分为：{f1_scores} \n')
        f.write(f'bagging-PSO-SVM模型在预测数据集中的AUC得分为：{auc_scores} \n')

    "4. 绘制预测，实际值" #二分变量不需要绘制
    ...
    "roc绘制"
    # ax = plt.gca()
    # ax.set_title(f'ROC for bagging-PSO-SVM on {date}')
    # svc_disp = RocCurveDisplay.from_estimator(bagging, predX, predy, ax=ax, alpha=0.7)
    # plt.savefig(f'/home/haozhic2/ListedCompany_risk/Results_output/Figure_file/Bagging_PSO_SVM_roc_{date}', dpi=300)
    # plt.show()

    "precison-recall 绘制方案"
    # display = PrecisionRecallDisplay.from_estimator(
    #     bagging, predX, predy, name="bagging-PSO-SVM"
    # )
    # _ = display.ax_.set_title(f" 2-class Precision-Recall curve for bagging-PSO-SVM on {date}")
    # plt.savefig(
    #     f'/home/haozhic2/ListedCompany_risk/Results_output/Figure_file/Bagging_PSO_SVM_precision_recall_{date}',
    #     dpi=300)
    # plt.show()

    "KS绘制"
    # plts = plt_ks(predy, y_pred_prob, 'bagging-PSO-SVM', date)  # 绘制KS 测试一下！
    # plts.savefig(f'/home/haozhic2/ListedCompany_risk/Results_output/Figure_file/Bagging_PSO_SVM_KSplot_{date}',
    #              dpi=300)

    "SHAP value的绘制工作"
    # explainer = shap.KernelExplainer(bagging.predict_proba, shap.kmeans(trainX,10), link='logit') # 增加shap.kmeans，加快速度
    ...
    # shap.summary_plot(shap_values[1], predX, title=f'SHAP value of bagging-PSO-SVM on {date} ')
    # plt.savefig(f'/home/haozhic2/ListedCompany_risk/Results_output/Figure_file/Bagging_PSO_SVM_shap_summary_{date}',
    #             dpi=300)
    # plt.show()
    #
    # return None
